...
public final class MqttBrokerService extends AbstractService<ByteBuffer, ByteBuffer> {
    ...
    private volatile ssg.legoflow.mqtt.broker.MqttBroker broker;
    ...
    protected void doConnect(ServiceContext ctx) {
        try {
            ...
            var config = ssg.legoflow.mqtt.broker.MqttBrokerConfig.defaults();
            ...
            this.broker = new ssg.legoflow.mqtt.broker.MqttBroker(config);
            broker.start();
        } catch (Exception e) {
    ...
    public ssg.legoflow.mqtt.broker.MqttBroker getBroker() { return broker; }
