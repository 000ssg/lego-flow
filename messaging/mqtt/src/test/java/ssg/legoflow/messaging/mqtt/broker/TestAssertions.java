...
/**
 * Retry-based assertion utilities for timing-sensitive MQTT tests.
 * Replaces blind {@code Thread.sleep()} with proper synchronization primitives
 * and exponential-backoff retries to eliminate flaky failures under parallel execution.
 */
final class TestAssertions {
     ...
     */
    static boolean waitForCondition(java.util.function.BooleanSupplier condition,
     ...
     */
    static <T> T waitFor(Callable<T> callable, Duration timeout, long initialDelay) {
        long deadline = System.currentTimeMillis() + timeout.toMillis();
        long delay = initialDelay;
        while (System.currentTimeMillis() < deadline) {
            try {
     ...
     */
    static void assertThatCondition(String description, java.util.function.BooleanSupplier condition,
                                   Duration timeout) {
     ...
     */
    static <T> void assertThatEventually(String description, Supplier<T> actualSupplier,
     ...
     */
    static void waitForFlag(AtomicBoolean flag, Duration timeout) {
