...
| §2.3 | Packet type values (1-14) | ✅ Implemented | `MqttPacketType` enum (CONNECT=1 through DISCONNECT=14); `MqttPacketTypeTest` |

### MQTT v5.0 — Additional Features

| Section | Requirement | Status | Verification |
|---------|------------|--------|-------------|
| §3.15 | AUTH packet (type 15) | ✅ Implemented | `AuthPacket`, `MqttPacketType.AUTH`; `MqttPacketTypeTest` |
| §2.2.2 | Properties (variable-length encoded) | ✅ Implemented | `MqttProperties`; `MqttPropertiesTest` |
...
| §3.8.3 | Subscription options (retain handling, no local, RAP) | ✅ Implemented | `TopicSubscription`, `RetainHandling`; `TopicSubscriptionTest` |
...
| Section | Requirement | Status | Verification |
|---------|------------|--------|-------------|
| §4.3.1 | QoS 0 — At most once delivery | ✅ Implemented | `QoS.AT_MOST_ONCE`; `QoSTest`, `QoSLevelsDemoTest` |
| §4.3.2 | QoS 1 — At least once delivery (PUBLISH/PUBACK) | ✅ Implemented | `QoS.AT_LEAST_ONCE`; `QoSTest`, `QoSLevelsDemoTest` |
| §4.3.3 | QoS 2 — Exactly once delivery (PUBLISH/PUBREC/PUBREL/PUBCOMP) | ✅ Implemented | `QoS.EXACTLY_ONCE`; `QoSTest`, `QoSLevelsDemoTest` |
...
| §4.7 | Wildcard subscription matching | ✅ Implemented | `TopicTree` with `TopicFilter`; `TopicTreeTest`, `WildcardTopicsDemoTest` |

### MQTT — Session Management

| Section | Requirement | Status | Verification |
|---------|------------|--------|-------------|
| §3.1.2.4 | Clean session flag | ✅ Implemented | `MqttSession`; `MqttSessionTest`, `SessionPersistenceDemoTest`, `CleanStartTest` |
| §3.1.2.4 | Persistent session (subscriptions survive disconnect) | ✅ Implemented | `MqttSession`; `MqttSessionTest`, `SessionPersistenceDemoTest`, `CleanStartTest` |
| §3.1.2.4 | Clean session (discard on disconnect) | ✅ Implemented | `MqttSession`; `MqttSessionTest`, `CleanStartTest` |
| §4.1 | Offline message queueing (QoS 1/2) | ✅ Implemented | `MqttSession`; `SessionPersistenceDemoTest` |
...
| §3.3.1.3 | Retained message storage | ✅ Implemented | `RetainStore`; `RetainStoreTest`, `RetainedMessageDemoTest` |
| §3.3.1.3 | Deliver retained message on new subscription | ✅ Implemented | `RetainStore`; `RetainedMessageDemoTest` |
...
- Total compliance tests: 193 (per CLAUDE.md)
- Key unit test classes: `MqttCodecTest`, `MqttEncoderTest`, `MqttDecoderTest`, `MqttPacketTypeTest`, `MqttPropertiesTest`, `QoSTest`, `TopicSubscriptionTest`, `TopicFilterTest`, `TopicTreeTest`, `MqttBrokerTest`, `MqttSessionTest`, `RetainStoreTest`, `MqttClientTest`, `MqttClientConfigTest`, `MqttTlsConfigTest`, `MqttAuthenticatorTest`, `SessionExpiryTest`, `CleanStartTest`, `DisconnectReasonCodeTest`, `QoSDowngradeTest`, `KeepAliveTimeoutTest`
- Key demo test classes: `SimplePubSubDemoTest`, `QoSLevelsDemoTest`, `RetainedMessageDemoTest`, `SessionPersistenceDemoTest`, `WildcardTopicsDemoTest`, `IoTSensorDemoTest`
