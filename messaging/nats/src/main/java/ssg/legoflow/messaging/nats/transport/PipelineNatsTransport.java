...
/**
 * {@link NatsTransport} implementation backed by a {@link DataChannel} in the service pipeline.
 *
 * <p>This is the production transport (mirrors the STOMP {@code PipelineTransport} reference
 * form). It owns <b>no sockets</b>: the channel is created, registered, and driven by the
 * {@code SelectableChannelManager} selector thread, and this class only moves bytes between
 * that channel and the protocol core.
 *
 ...
 * write happens on the selector thread in {@link #onWrite} (the STOMP/MQTT rule — never write
 * the channel from the calling thread).
 *
 * <p>One instance per connection.
 *
 * @since 0.1.0
 */
public final class PipelineNatsTransport implements NatsTransport {

    private static final Logger LOG = LoggerFactory.getLogger(PipelineNatsTransport.class);
    ...
    public PipelineNatsTransport(DataChannel channel) {
    ...
    public void send(ByteBuffer data) {
        ...
        // its read/write and causes flaky message loss (the STOMP/AMQP/MQTT rule).
