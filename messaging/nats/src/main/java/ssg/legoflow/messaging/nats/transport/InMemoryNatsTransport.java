...
/**
 * In-memory {@link NatsTransport} pair for testing the NATS protocol without TCP sockets.
 *
 ...
 * @since 0.1.0
 */
public final class InMemoryNatsTransport implements NatsTransport {

    private final BlockingQueue<ByteBuffer> inbound;
    private final BlockingQueue<ByteBuffer> outbound;
    private final AtomicBoolean open = new AtomicBoolean(true);
    private volatile InMemoryNatsTransport peerRef;

    private InMemoryNatsTransport(BlockingQueue<ByteBuffer> inbound, BlockingQueue<ByteBuffer> outbound) {
     ...
     */
    public static InMemoryNatsTransport[] createPair() {
        var q1 = new LinkedBlockingQueue<ByteBuffer>();
        var q2 = new LinkedBlockingQueue<ByteBuffer>();
        var a = new InMemoryNatsTransport(q1, q2);
        var b = new InMemoryNatsTransport(q2, q1);
        a.peerRef = b;
        b.peerRef = a;
        return new InMemoryNatsTransport[]{a, b};
    }
    ...
    public int receiveWithTimeout(ByteBuffer buffer, long timeout, TimeUnit unit) {
        ...
        while (true) {
            ...
            // still readable (mirrors a socket delivering buffered data before EOF). This
            // ensures a server's -ERR is not lost when it closes the transport right after.
