...
/** Tests for PostgreSQL server service DP/DF compliance. */
class PgServerServiceTest {

    @Test void testBuilderCreatesService() {
        var service = PgServerService.builder(5432).build();
        assertThat(service).isNotNull();
    }

    @Test void testInitialStateIsDisconnected() {
        var service = PgServerService.builder(0).build();
    ...
    @Test void testBuilderWithPriority() {
        var service = PgServerService.builder(5432).priority(80).build();
