...
- [ ] Negotiation/Auth sub-category (9 version sub-tasks) — v0 ✓ (`3f42231e`): SaslHandshake v0, ApiVersions v0, SaslAuthenticate v0; v1 ✓ (`2f2d5ba8`): SaslHandshake v1 (byte-identical → shared v0 path), ApiVersions v1 (response +ThrottleTimeMs int32), SaslAuthenticate v1 (response +SessionLifetimeMs int64) — 441 tests green; v2 ✓ (this session): ApiVersions v2 (byte-identical vs v1 → shared v1 path, v2 only changes throttle semantics); 2 rows remaining (ApiVersions v3 — flexible encoding, SaslAuthenticate v2 — flexible encoding)
